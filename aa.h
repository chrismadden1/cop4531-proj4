...
namespace fsu
{
  ...
  template <typename K, typename D, class H = hashclass::KISS<fsu::String>>
  class HashTable;
  ...
  private:
    ...
    size_t                 size_; // added via project specs.
  ...
  */
  ...
  {
    size_t temp = 0, checkSize = 0;
    for(size_t i = 0; i < numBuckets_; i++){
      temp = bucketVector_[i].Size();
      checkSize += temp;
    }
  //  cout << checkSize << "    " << size_ << "\n";
    if(checkSize == size_)
      return true;
    else
      return false;
